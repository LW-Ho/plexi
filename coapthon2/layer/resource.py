...
class ResourceLayer(object):
	...
	def edit_resorce(self, request, response, node, lp, p):
		...
		if hasattr(method, '__call__'):
			...
			if resource and resource.path != p:
				child_resource = resource
				child_node = node.add_child(child_resource)
				self._parent.notify(child_node)
				resource = None
			...
			if new_payload is not None and new_payload != -1:
				...
				if request.content_type is not None and request.content_type in defines.content_types:
					resource.raw_payload[request.content_type] = node.value.raw_payload[request.content_type]
				else:
					resource.raw_payload[defines.inv_content_types["text/plain"]] = node.value.raw_payload[defines.inv_content_types["text/plain"]]
				...
				if request.accept == defines.inv_content_types['application/json']:
					if isinstance(new_payload, str):
						response.payload = new_payload
					else:
						response.payload = json.dumps(new_payload)
				else:
					response.payload = new_payload
				# Token
