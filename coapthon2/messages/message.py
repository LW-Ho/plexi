...
class Message(object):
	...
	def csv(self):
		msg = ''
		if self.source:
			msg = str(self.source[0]) + ':' + str(self.source[1]) + ","
		else:
			msg += str(self.source) + ','
		if self.destination:
			msg += str(self.destination[0]) + ':' + str(self.destination[1]) + ","
		else:
			msg += str(self.destination) + ','
		msg += str(defines.types[self.type]) + ","
		msg += str(self.token) + ","
		msg += str(self.mid) + ","
		if self.code is None:
			self.code = 0
		try:
			msg += str(defines.inv_responses[self.code]) + ","
		except KeyError:
			msg += str(defines.codes[self.code]) + ","
		uripath = ''
		c = 0
		for opt in self._options:
			if opt.name == 'Uri-Path':
				uripath += str(opt.value)+'/'
				c += 1
		msg += uripath + ','
		for opt in self._options:
			if opt.name != 'Uri-Path':
				msg += opt.csv()
				if c<len(self._options)-1:
					msg += '~'
				else:
					msg += ','
				c += 1
		msg += str(self.payload).strip()
		return msg