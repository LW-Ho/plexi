...
class Resource(object):
	...
	def __init__(self, name, visible=True, observable=True, allow_children=True):
		...
		if isinstance(name, Resource):
			...
			self._required_content_type = name._required_content_type
			self._allow_children = name.allow_children
			self.observe_count = name.observe_count
			self._payload = name._payload
	...
	def payload(self):
		...
		if self._required_content_type is not None:
			if isinstance(self._payload, dict):
				try:
					p = self._payload[self._required_content_type]
					if self._required_content_type == defines.inv_content_types['application/json']:
						return json.dumps(p)
					return p
				except KeyError:
					return -2
			...
			if isinstance(self._payload, dict):
				...
				elif defines.inv_content_types["application/json"] in self._payload:
					return json.dumps(self._payload[defines.inv_content_types["application/json"]])
				else:
